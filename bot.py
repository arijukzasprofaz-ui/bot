...
async def send_result(ctx, msg, result, filename):
    if len(result) <= 1900:
...
async def cmd_deob(ctx):
    ...
    if not code:
        ...
        return
    obf = detect_obfuscator(code)
    msg = await ctx.reply(f"⏳ Detected **{obf}** — reconstructing...")
    result = await asyncio.to_thread(run_deob, code, False)
    await send_result(ctx, msg, result, "deobf.lua")
...
async def cmd_deob_full(ctx):
    ...
    if not code:
        ...
        return
    obf = detect_obfuscator(code)
    msg = await ctx.reply(f"⏳ Detected **{obf}** — full reconstruction...")
    result = await asyncio.to_thread(run_deob, code, True)
    await send_result(ctx, msg, result, "deobf_full.lua")
...
async def cmd_help(ctx):
    ...
    embed.add_field(name="!deob / !d", value="Detect and reconstruct supported code", inline=False)
    embed.add_field(name="!deob_full / !df", value="Full reconstruction + constants/instruction detail", inline=False)
